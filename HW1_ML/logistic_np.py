...
def normalize_per_pixel(train_x, test_x):
    ...
    train_mean = np.zeros((1, R, C))
    train_std = np.zeros((1, R, C))

    train_mean = np.sum(train_x, axis=0)/m

    for i in range(m):
        train_std += (train_x[i] - train_mean)**2
    train_std = np.sqrt(train_std / m)
...
def reshape2D(tensor):
    ...
    tensor = np.reshape(tensor, (m, R*C))
...
if __name__ == "__main__":
    ...
    # print("train_x's shape = {}\ntest_x's shape = {}".format(train_x.shape, test_x.shape))
